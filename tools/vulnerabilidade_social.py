...
class BolsaFamilia(BaseModel):
    familias_beneficiadas: int = Field(..., description="Número de famílias beneficiadas pelo Bolsa Família")
    valor_medio: float = Field(..., description="Valor médio do benefício por família")

class Saneamento(BaseModel):
    agua_encanada: float = Field(..., description="Percentual de domicílios com água encanada")
    coleta_esgoto: float = Field(..., description="Percentual de domicílios com coleta de esgoto")
    coleta_lixo: float = Field(..., description="Percentual de domicílios com coleta de lixo")

class Emprego(BaseModel):
    taxa_informalidade: float = Field(..., description="Taxa de informalidade no mercado de trabalho")
    desemprego: float = Field(..., description="Taxa de desemprego")
    microempresas_ativas: int = Field(..., description="Número de microempresas ativas")

class Educacao(BaseModel):
    escolaridade_media: float = Field(..., description="Escolaridade média em anos")
    taxa_analfabetismo: float = Field(..., description="Taxa de analfabetismo")

class SaudeBasica(BaseModel):
    cobertura_aps: float = Field(..., description="Cobertura de Atenção Primária à Saúde")

class Seguranca(BaseModel):
    homicidios_ano: int = Field(..., description="Número de homicídios no ano")
    violencia_domestica: int = Field(..., description="Número de casos de violência doméstica")

class VulnerabilidadeSocial(BaseModel):
    municipio: str = Field(..., description="Nome do município e UF")
    ano: int = Field(..., description="Ano de referência dos dados")
    familias_vulneraveis: int = Field(..., description="Número de famílias em situação de vulnerabilidade")
    extrema_pobreza: int = Field(..., description="Número de pessoas em situação de extrema pobreza")
    bolsa_familia: BolsaFamilia = Field(..., description="Dados do Bolsa Família")
    saneamento: Saneamento = Field(..., description="Dados de saneamento básico")
    emprego: Emprego = Field(..., description="Dados de emprego e renda")
    educacao: Educacao = Field(..., description="Dados educacionais")
    saude_basica: SaudeBasica = Field(..., description="Dados de saúde básica")
    seguranca: Seguranca = Field(..., description="Dados de segurança pública")
...
async def obter_vulnerabilidade_social(municipio: str, ano: int = 2023) -> Dict[str, Any]:
    ...
    resultado = {
        ...
        "bolsa_familia": {
            "familias_beneficiadas": dados_cadunico.get("familias_beneficiadas", 0),
...
async def _obter_dados_demograficos(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        nome_municipio = f"{dados_municipio.get('nome')} - {dados_municipio.get('microrregiao', {}).get('mesorregiao', {}).get('UF', {}).get('sigla', '')}"
...
async def _obter_dados_cadunico(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        dados = await cadunico.obter_dados_cadunico(codigo_municipio, ano)
...
async def _obter_dados_saneamento(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        dados = await snis.obter_dados_saneamento(codigo_municipio, ano)
...
async def _obter_dados_emprego(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        dados = await cnpj.obter_dados_emprego(codigo_municipio, ano)
...
async def _obter_dados_educacao(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Obtém dados educacionais
        dados = await educacao.obter_dados_educacionais(codigo_municipio, ano)
...
async def _obter_dados_saude(municipio: str, ano: int) -> Dict[str, Any]:
...
async def _obter_dados_seguranca(municipio: str, ano: int) -> Dict[str, Any]:
    ...
    try:
        ...
        dados = await seguranca.obter_dados_seguranca(codigo_municipio, ano)
