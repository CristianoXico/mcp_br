...
from .ibge_base import make_request, get_cached_data, save_to_cache, logger, BASE_URL_CNAE
...
def pesquisar_cnae(termo: str) -> List[Dict]:
    ...
    if cached_data:
        return cached_data
    try:
        url = f"{BASE_URL_CNAE}/pesquisa/{termo}"
        resultados = make_request(url)
        return save_to_cache(cache_key, resultados)
    except Exception as e:
        logger.error(f"Erro ao pesquisar CNAE pelo termo '{termo}': {e}")
        return [{"erro": str(e)}]
